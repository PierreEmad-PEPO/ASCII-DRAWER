...
vector<myPoint> rotation_before(1);
bool rotating=0;
int r1=_select[1].x-_select[0].x,c1=_select[1].y-_select[0].y;
int rotation_i,rotation_j;
vector<myPoint> rotated_vect(1);
bool break_inrotation=0;
int temp_rotate_x,temp_rotate_y;
int local_x,local_y;
char print,cursor_char='0';
bool exported_ss=0,clear_flag=0;
vector<myPoint> undo(1);
vector<myPoint> cpd(1);
myPoint cpy;
...
void input()
{
    ...
    case ' ':
        ...
        case 'c':
            ...
            {
                for(int i=_select[0].x;i<=_select[1].x;i++)
                {
                    for(int j=_select[0].y;j<=_select[1].y;j++)
                    {
...

{
        ...
        case '0':
            ...
            if(select_count>=2)
            {
                ...
                if(_select[0].y>_select[1].y)
                    {
                        swap(_select[0].y,_select[1].y);
                    }

                last_crsr_x=_select[0].x,last_crsr_y=_select[0].y;
                cursor = _select[0];


                for(int i=_select[0].x;i<=_select[1].x;i++)
                {
                    for(int j=min(_select[0].y,_select[1].y);j<=max(_select[0].y,_select[1].y);j++)
                    {
        ...
        case '+':
            ...
            r1=_select[1].x-_select[0].x,c1=_select[1].y-_select[0].y;
            rotating=1;
    ...
    for(int cntt=0;cntt<selected_pressed.size();cntt++)
            {
                ...
                pressed[selected_pressed[cntt]].x=_select[0].x+(  temp_rotate_y  -_select[0].y);
                pressed[selected_pressed[cntt]].y=_select[0].y+r1 -(  temp_rotate_x  -_select[0].x);
            }
        ...
        case 'f':
            ...
            {
                pressed[selected_pressed[ct]].y = _select[1].y - pressed[selected_pressed[ct]].y + _select[0].y;
            }
            ...
            for(int ct=0;ct<selected_pressed.size();ct++)
            {
                pressed[selected_pressed[ct]].x = _select[1].x - pressed[selected_pressed[ct]].x + _select[0].x;
            }
    ...
    if(confirm_select && one_step)
                {
                    ...
                    _select[0].x+=(cursor.x-last_crsr_x);
                    _select[0].y+=(cursor.y-last_crsr_y);
                    _select[1].x+=(cursor.x-last_crsr_x);
                    _select[1].y+=(cursor.y-last_crsr_y);
                    one_step=0;
                }
...
void draw(vector<myPoint> pressed)
{
    int cnt=0;
    ...
    for(i=0;i<rows;i++)
    {
        ...
        for(j=0;j<columns;j++)
        {
            ...
            else
            {
                ...
                if(b)
                {
                        if(selecting && i>=min(_select[0].x,cursor.x)&&i<=max(_select[0].x,cursor.x) &&
                            j>=min(_select[0].y,cursor.y)&&j<=max(_select[0].y,cursor.y))
                            {
                                cout<<'.';
                            }
                    else if( confirm_select &&i>=min(_select[0].x,_select[1].x)&&i<=max(_select[0].x,_select[1].x) &&
                            j>=min(_select[0].y,_select[1].y)&&j<=max(_select[0].y,_select[1].y))
                            {
                                cout<<'.';
                            }
                    else
